...
For reproducibility, pin a version (e.g. `spytial-core@4.4.2`).
...
| **React components** | `CndLayoutInterface`, `ErrorMessageContainer`, `ProjectionControls`, `ProjectionOrchestrator`, `ReplInterface`, `ReplWithVisualization` — via `spytial-core/react` (npm) or the components CDN bundle; kept out of the default entry since 4.0.0. `InstanceBuilder` is deprecated here; use the `<structured-input-graph>` custom element. |
