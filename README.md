...
| **React components** | `CndLayoutInterface`, `ErrorMessageContainer`, `ReplInterface`, `PyretReplInterface`, `ProjectionControls`, `ProjectionOrchestrator` — via `spytial-core/react` (npm) or the components CDN bundle; kept out of the default entry since 4.0.0. To author a data instance, use the `<structured-input-graph>` custom element rather than a React component. |
