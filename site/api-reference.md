...
| Tag                     | Class                | Role                                                                          |
|-------------------------|----------------------|-------------------------------------------------------------------------------|
| `<webcola-cnd-graph>`   | `WebColaCnDGraph`    | The default visual renderer. Methods: `renderLayout`, `generateSequenceLayouts`, `getLayoutState`, `getNodePositions`, `clear`, `highlightNodes`, `highlightNodePairs`, `clearNodeHighlights`, `getAllRelations`, `highlightRelation`, `clearHighlightRelation`, `resetViewToFitContent`, `addToolbarControl`, `takeScreenshot(filename?)` (downloads a PNG, same as the toolbar button), `exportPng({ scale? })` (returns the PNG as a `Blob`, no download). |
...
For reproducibility, pin a version (`spytial-core@6.0.1`).
