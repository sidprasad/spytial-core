...
Decide:
- **Bundle source.** NPM (`spytial-core`) or CDN? **If CDN, pin a version** — `spytial-core@5.4.2` over bare `spytial-core`. Papers, locked notebooks, and reproducibility-sensitive workflows depend on this; bare CDN URLs silently shift under users. The README has the canonical pinned URL.
...
2. **Wire to a minimal HTML harness.** Use the core pipeline from `README.md` (the five-stage `JSONDataInstance` → `parseLayoutSpec` → `SGraphQueryEvaluator` → `LayoutInstance` → `renderLayout` flow). Load the bundle from a *pinned* CDN version (e.g. `spytial-core@5.4.2`) for the prototype.
